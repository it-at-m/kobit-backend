...
public interface ContactPointRepository extends JpaRepository<ContactPoint, UUID> {
    ...
    @Query(value = "SELECT p.id, p.name, p.short_cut, p.description, d.department FROM "
                        + " {h-schema}contact_point as p LEFT JOIN {h-schema}departments as d ON p.id ="
    ...
    @Query(
            nativeQuery = true,
            value =
                    "SELECT * FROM {h-schema}contact_point as p LEFT JOIN {h-schema}departments d on p.id ="

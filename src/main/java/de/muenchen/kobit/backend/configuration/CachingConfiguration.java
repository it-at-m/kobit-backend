 ...
 * the corresponding bean creation methods or remove the annotation {@link EnableCaching} above the
 * class definition.
 */
@Configuration
@EnableCaching
public class CachingConfiguration {
    ...
    /**
     * The config to provide a cache for method {@link
     * CustomUserInfoTokenServices#loadAuthentication(String)}.
     *
    ...
    public Cache authenticationCache(Ticker ticker) {
        return new CaffeineCache(
                ...
                Caffeine.newBuilder()
                        .expireAfterWrite(
                                AUTHENTICATION_CACHE_EXPIRATION_TIME_SECONDS, TimeUnit.SECONDS)
                        .ticker(ticker)
                        .build());
    }
}

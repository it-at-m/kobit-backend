...
public class ReportEmailService {
    ...
    public void sendViewCounterReport(List<ViewCounter> counters, YearMonth reportMonth)
        ...
        if (counters != null && !counters.isEmpty()) {
                    ...
                    .forEach(
                            vc -> {
                                String cat =
                                        vc.getCategory() == null
                                                ? "-"
                                                : escapeHtml(vc.getCategory().getDescription());
            ...
            rows.append(
                    "<tr><td colspan='2' style='padding:8px;border:1px solid"
                            + " #e5e7eb;text-align:center;color:#6b7280'>Keine Daten</td></tr>");
        }

        String html =
                "<div style=\"font-family:system-ui,Segoe"
                        + " UI,Roboto,Arial,sans-serif;font-size:14px;color:#111827\"><h3"
                        + " style=\"margin:0 0 8px 0\">UFO-Aufrufzahlen - Report</h3><p"
                        + " style=\"margin:0 0 12px 0;color:#374151\">Berichtsmonat: <strong>"
                        + monthLabel
                        + "</strong></p><table"
                        + " style=\"width:100%;border-collapse:collapse;border:1px solid"
                        + " #e5e7eb\"><thead><tr style=\"background:#f9fafb\"><th align=\"left\" "
                        + " style=\"padding:8px;border:1px solid"
                        + " #e5e7eb;text-transform:uppercase;font-size:12px;color:#6b7280\">Kategorie</th><th"
                        + " align=\"right\" style=\"padding:8px;border:1px solid"
                        + " #e5e7eb;text-transform:uppercase;font-size:12px;color:#6b7280\">Aufrufe</th>"
                        + "</tr></thead><tbody>"
                        + rows
                        + "</tbody></table></div>";

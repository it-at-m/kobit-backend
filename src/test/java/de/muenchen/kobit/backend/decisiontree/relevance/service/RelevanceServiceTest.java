...
class RelevanceServiceTest {
    ...
    private final RelevanceCompetenceRepository relevanceCompetenceRepository =
            mock(RelevanceCompetenceRepository.class);
    ...
    void init() {
        service =
                new RelevanceService(
                        relevanceRepository, pathRepository, relevanceCompetenceRepository);
    }
    ...
    void getOrderTest() {
        ...
        final Set<Competence> competences =
                Set.of(Competence.DISCRIMINATION, Competence.ETHNIC_RACIAL);
        final RelevanceCompetence discrimination =
                new RelevanceCompetence(Competence.DISCRIMINATION.toString());
        final RelevanceCompetence ethnic =
                new RelevanceCompetence(Competence.ETHNIC_RACIAL.toString());
        final RelevanceCompetence antiDemocracy =
                new RelevanceCompetence(Competence.ANTI_DEMOCRACY.toString());
        final List<Path> paths =
                List.of(
                        new Path(pathId, Set.of(discrimination, ethnic)),
                        new Path(Set.of(antiDemocracy)));
        when(pathRepository.findAll()).thenReturn(paths);
        when(relevanceRepository.findAllByPathId(pathId))
                .thenReturn(List.of(new Relevance(contactPointId, pathId, 1)));
